  ...
  angular.module('ImageCropper',[])
    .directive('imageCrop', function() {
      ...
      return {
        ...
        '<section ng-style="sectionStyles" ng-show="step==1"></section>' +
        '<section ng-style="sectionStyles" ng-show="step==2">' +
        '<canvas class="cropping-canvas" width="{{ canvasWidth }}" height="{{ canvasHeight }}" ng-mousemove="onCanvasMouseMove($event)" ng-mousedown="onCanvasMouseDown($event)"></canvas>' +
        '<div ng-style="croppingGuideStyles" class="cropping-guide"></div><div class="zoom-handle" ng-mousemove="onHandleMouseMove($event)" ng-mousedown="onHandleMouseDown($event)" ng-mouseup="onHandleMouseUp($event)">' +
        '<span>&larr; zoom &rarr;</span>' +
        '</div>' +
        '</section>' +
        '<section ng-style="sectionStyles" class="image-crop-section-final" ng-show="step==3">' +
        '<img class="image-crop-final" ng-src="{{ croppedDataUri }}" /></section></div>',
        replace: true,
        restrict: 'AE',
        scope: {
		  crop: '=',
