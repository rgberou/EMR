...
(function(window, angular, undefined) {
  ...
  angular.module('satellizer', [])
    .constant('SatellizerConfig', {
      ...
      tokenName: 'token',
      tokenPrefix: 'satellizer', //accesssoft
      ...
      this.$get = [
        ...
        function($q, shared, local, oauth) {
          ...
          $auth.setCookie = function(cookiename,data) {
            return shared.setCookie(cookiename, data)
          };

          $auth.removeCookie = function(cookiename) {
            return shared.removeCookie(cookiename);
          };
    ...
    .factory('SatellizerShared', [
      ...
      '$rootScope',
      'Base64',
      function($q, $window, $log, utils, config, storage, $cookieStore, $rootScope, Base64) {
        var Shared = {};
        ...
        Shared.setCookie  = function (cookiename,data) {
          var datas = {};
          datas.dataObj = data;
          for(var key in datas.dataObj){
            if(!datas.dataObj[key]){
              datas.dataObj[key] = 'null';
            }
            datas.dataObj[key] = Base64.encode(datas.dataObj[key]);
          }
          $cookieStore.put(cookiename, datas);
        };

        Shared.removeCookie = function (cookiename) {
