...
(function(window, document, undefined) {
  ...
  angular.module('mgcrea.ngStrap.modal', [ 'mgcrea.ngStrap.core', 'mgcrea.ngStrap.helpers.dimensions' ]).provider('$bsModal', function() {
    ...
    this.$get = [ '$window', '$rootScope', '$bsCompiler', '$animate', '$timeout', '$sce', 'dimensions', function($window, $rootScope, $bsCompiler, $animate, $timeout, $sce, dimensions) {
      ...
      function ModalFactory(config) {
        ...
        var promise = $bsModal.$promise = $bsCompiler.compile(options);
        var scope = $bsModal.$scope = options.scope && options.scope.$new() || $rootScope.$new();
        if (!options.element && !options.container) {
          options.container = 'body';
        }
        $bsModal.$id = options.id || options.element && options.element.attr('id') || '';
        ...
        $bsModal.$isShown = scope.$isShown = false;
        ...
        $bsModal.show = function() {
          ...
          modalScope = $bsModal.$scope.$new();
          modalElement = $bsModal.$element = compileData.link(modalScope, function(clonedElement, scope) {});
          ...
          $bsModal.$isShown = scope.$isShown = true;
          safeDigest(scope);
        ...
        $bsModal.toggle = function() {
          $bsModal.$isShown ? $bsModal.hide() : $bsModal.show();
        };
        $bsModal.focus = function() {
          modalElement[0].focus();
        };
        $bsModal.$onKeyUp = function(evt) {
          if (evt.which === 27 && $bsModal.$isShown) {
            $bsModal.hide();
            evt.stopPropagation();
          }
        };
        ...
        function destroyModalElement() {
          if ($bsModal.$isShown && modalElement !== null) {
            unbindBackdropEvents();
            unbindKeyboardEvents();
          }
          if (modalScope) {
          ...
          if (modalElement) {
            modalElement.remove();
            modalElement = $bsModal.$element = null;
          }
        }
        return $bsModal;
      }
  ...
  angular.module('mgcrea.ngStrap.alert', [ 'mgcrea.ngStrap.modal' ]).provider('$alert', function() {
    ...
    this.$get = [ '$bsModal', '$timeout', function($bsModal, $timeout) {
      function AlertFactory(config) {
        var $alert = {};
        var options = angular.extend({}, defaults, config);
        $alert = $bsModal(options);
